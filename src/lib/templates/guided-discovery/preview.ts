...
const discoveryMathSection: SectionContent = {
  ...
  header: {
    ...
    subtitle:
      "When two things happen in sequence — how do the odds combine?",
    subject: "Mathematics",
    grade_band: "secondary",
    objective:
      "Use tree diagrams and the multiplication rule to calculate compound probabilities"
  },

  hook: {
    headline:
      "If you flip two coins, why is getting two heads not twice as hard as getting one?",
  ...
  definition: {
    ...
    formal:
      "For independent events A and B, P(A ∩ B) = P(A) · P(B). Events are independent if the occurrence of one does not change the probability of the other.",
...
draw();
    ...
    fallback_diagram: {
      ...
      caption:
        "Probability tree for two fair coin flips showing four equally likely outcomes, each with probability ¼.",
      alt_text:
  ...
  worked_example: {
    ...
    setup:
      "You roll two fair six-sided dice. What is the probability of rolling doubles (both dice show the same number)?",
    steps: [
      {
        label: "Identify the events",
        content:
          "Die 1 shows any number (6 outcomes). Die 2 must match die 1 (1 favourable out of 6)."
      },
      {
        label: "Check independence",
        content:
          "The dice are independent — the result of one does not affect the other."
      },
      {
        label: "Apply the multiplication rule",
        content:
          "P(die 1 = k) = 1/6 for any k. P(die 2 = k | die 1 = k) = 1/6. But we need to sum over all k: P(doubles) = 6 × (1/6 × 1/6) = 6/36 = 1/6."
      }
    ],
    conclusion:
      "There are 6 favourable outcomes (1-1, 2-2, ... 6-6) out of 36 total, giving P(doubles) = 1/6 ≈ 16.7%.",
    answer: "1/6 ≈ 0.167"
  },
  ...
  pitfall: {
    ...
    correction:
      "P(at least one head in two flips) = ¾, not ½. P(two heads) = ¼. The relationship isn't linear — it's multiplicative. Each additional requirement multiplies, not adds, to reduce the probability.",
    example:
      "Getting at least one head from two coins: P = 1 − P(TT) = 1 − ¼ = ¾. That's more likely than a single head from one coin (½).",
  ...
  reflection: {
    prompt:
      "How does the tree diagram help you see where the multiplication rule comes from? Could you explain it to someone who only knows single-event probability?",
    type: "open",
    space: 4
  },
    ...
    problems: [
      {
        difficulty: "warm",
        question:
          "A coin is flipped and a die is rolled. What is the probability of getting heads AND rolling a 4?",
        hints: [
          {
            level: 1,
            text: "Are these events independent? If so, multiply their probabilities."
          },
          {
            level: 2,
            text: "P(H) = ½, P(4) = 1/6. Multiply them."
          }
        ],
        solution: {
      ...
      },
      {
        difficulty: "medium",
        question:
          "A bag has 3 red and 5 blue marbles. You draw one marble, replace it, then draw again. What is the probability of drawing red both times?",
        hints: [
          {
            level: 1,
            text: "With replacement means the draws are independent."
          },
          {
            level: 2,
            text: "P(red) = 3/8 each time. Apply the multiplication rule."
          }
        ],
        solution: {
          approach:
            "Independent draws with replacement — multiply probabilities.",
      ...
      },
      {
        difficulty: "cold",
        question:
          "You flip a coin three times. Draw the full probability tree and find P(exactly two heads).",
        hints: [
          {
            level: 1,
            text: "The tree has 8 final branches (2³ = 8)."
          },
          {
            level: 2,
            text: "Count paths with exactly two H: HHT, HTH, THH."
          },
          {
            level: 3,
            text: "Each path has probability ½ × ½ × ½ = 1/8. There are 3 such paths."
          }
        ],
        solution: {
          approach:
            "List all paths with exactly 2 heads and sum their probabilities.",
    ...
    terms: [
      {
        ...
        definition:
          "Two events where the outcome of one does not affect the probability of the other.",
      ...
      },
      {
        term: "Compound Event",
        definition:
          "An event consisting of two or more simple events occurring together or in sequence.",
        related: ["Multiplication Rule", "Sample Space"]
      },
      {
        term: "Probability Tree",
        definition:
          "A diagram where each branch represents a possible outcome, with probabilities on each branch.",
      ...
      },
      {
        ...
        definition:
          "The probability of an event given that another event has occurred: P(A|B).",
  ...
  what_next: {
    ...
    preview:
      "Drawing without replacement, Bayes' theorem, and the difference one condition makes.",
    prerequisites: [
...
export const guidedDiscoveryPreview: TemplatePreview = {
  section: discoveryMathSection,
  summary:
    "A probability section that explains the multiplication rule first, then lets learners build their own probability tree in the simulation to verify the theory."
};
