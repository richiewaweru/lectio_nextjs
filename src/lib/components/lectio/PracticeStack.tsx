...
function PracticeProblemBody({
  ...
  problem: PracticeContent["problems"][number];
  revealedHintCount: number;
  assessment: "matched" | "review" | undefined;
  onRevealHint: () => void;
  onSetAssessment: (value: "matched" | "review") => void;
}) {
    ...
    <div className="space-y-4">
      ...
      {problem.self_assess ? (
        <div className="rounded-[1rem] border border-border/70 bg-background/80 p-4">
          <p className="text-sm font-semibold text-primary">Self-check</p>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Button
              ...
              size="sm"
              onClick={() => onSetAssessment("matched")}
            >
              Matched
            </Button>
            <Button
              ...
              size="sm"
              onClick={() => onSetAssessment("review")}
            >
              Needs review
            </Button>
          </div>
              ...
              <AccordionItem key={index} value={`problem-${index}`} className="bg-white/80">
                <AccordionTrigger>
                  <div className="flex items-start gap-3 pr-4">
                    ...
                    <span className="leading-7 text-foreground/88">{problem.question}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent>
                  <PracticeProblemBody
                    problem={problem}
                    revealedHintCount={revealedHints[index] ?? 0}
                    assessment={selfAssessments[index]}
                    onRevealHint={() =>
                      revealNextHint(index, normalizePracticeHints(problem).length)
                    }
                    onSetAssessment={(value) =>
            ...
            {content.problems.map((problem, index) => (
              ...
              >
                <div className="mb-4 flex items-start gap-3">
                  ...
                  <p className="leading-7 text-foreground/88">{problem.question}</p>
                </div>
                <PracticeProblemBody
                  problem={problem}
                  revealedHintCount={revealedHints[index] ?? 0}
